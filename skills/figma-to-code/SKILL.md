...
4. **텍스트 내용 (원문 그대로)** — 실제 텍스트 콘텐츠. **Figma의 텍스트를 임의로 변경, 요약, 의역하지 않는다.** MCP 출력 코드의 텍스트 노드에 있는 문구를 한 글자도 바꾸지 않고 그대로 사용한다. 줄바꿈(`<br>`) 위치도 정확히 유지한다.
5. **카드/리스트 내부 구조** — 카드형 컴포넌트 내부에 **아이콘/이미지가 포함되어 있는지 반드시 확인**한다:
   - MCP 출력에서 카드 안의 `<img>` 태그와 에셋 URL을 놓치지 않는다
   - 아이콘+텍스트가 **가로 배치(flex-row)**인지 **세로 배치(flex-column)**인지 확인한다
   - 아이콘의 크기(width/height), border-radius(원형 여부), 배경색을 정확히 추출한다
6. **인스턴스별 차이** — 동일 컴포넌트가 반복될 때 각 인스턴스의 개별 속성:
...
7. **CSS 효과** — `backdrop-filter`, `opacity`, `rgba` 배경 등 시각 효과 추출

**⚠️ 텍스트 콘텐츠 정확성은 최우선이다.** MCP 출력의 텍스트를 "더 자연스럽게" 바꾸거나 요약하면 디자인과 불일치가 발생한다. Figma 디자인의 텍스트는 기획자/디자이너가 의도한 정확한 문구이므로 반드시 원문 그대로 사용한다.

**⚠️ 카드형 컴포넌트의 내부 에셋을 누락하지 않는다.** 피처 카드, 서비스 설명 카드 등에는 흔히 아이콘 이미지가 포함된다. MCP 출력에서 `component/Body/T2` 같은 카드 컴포넌트 내부의 이미지 노드를 반드시 확인하고, 에셋 URL을 추출하여 적용한다. 아이콘 없이 텍스트만으로 카드를 구성하면 디자인과 크게 달라진다.
...
- 정렬 방식: justify-content, align-items (예: justify-center, justify-between, items-center)
- flex-wrap 여부: (예: flex-wrap vs nowrap — 반응형 레이아웃에 영향)
...
- 카드 내부 구조: (예: flex-row + icon(84x84,rounded-full) + text-group)
- 텍스트 원문: 각 텍스트 노드의 정확한 문구와 줄바꿈 위치
```

**⚠️ justify-content는 섹션마다 다를 수 있다.** 헤더가 `space-between`이라고 서브 탭도 `space-between`이라 가정하지 않는다. 각 섹션의 MCP 출력에서 개별 확인한다.
...
/* 아이콘+텍스트 가로 카드 (component/Body/T2 패턴) */
/* ⚠️ 카드 내부에 아이콘 이미지가 있는지 반드시 MCP 출력을 확인한다 */
.card-horizontal {
  display: flex;
  gap: 16px;
  align-items: center;
...
/* 수직 컬럼 타임라인 — "컬럼별 아이템" 구조 (가로 행 분할 금지) */
/*
 * ⚠️ 핵심 원칙: 타임라인은 반드시 "컬럼별 아이템(flex-column per item)" 구조로 구현한다.
 *
 * ❌ 잘못된 구조 (가로 행 3줄 분할):
 *   [아이콘 행] → [도트 행] → [텍스트 행]  ← 이렇게 하면 안 된다!
 *   이 방식은 아이템별 높이 차이를 표현할 수 없고, 도트-텍스트 연결이 끊어진다.
 *
 * ✅ 올바른 구조 (컬럼별 아이템):
 *   [아이템1(컬럼)] [아이템2(컬럼)] [아이템3(컬럼)] ...
 *   각 아이템이 독립적인 flex-column으로 아이콘→연도→도트→커넥터→텍스트를 포함한다.
 *
 * ⚠️ 아이템별 height가 다르다고 지그재그(위/아래 교차) 패턴이 아니다!
 *   모든 아이템이 같은 방향(위→아래)으로 흐르되 커넥터 길이만 다른 것이다.
 *   MCP 데이터에서 각 아이템의 개별 height를 추출하여 그대로 적용한다.
 */
.timeline {
  display: flex;
  align-items: flex-start;  /* 상단 정렬 — 아이콘/연도는 같은 높이, 텍스트는 커넥터 길이에 따라 다른 위치 */
  justify-content: space-between;
  position: relative;  /* 수평 연결선의 기준 */
}
.timeline-item {
  display: flex;
  flex-direction: column;  /* 세로 배치: 아이콘→연도→도트→커넥터→텍스트 */
  align-items: center;
  width: 153px;  /* MCP 데이터에서 추출한 고정 폭 */
  /* ⚠️ height는 각 아이템마다 MCP 데이터의 개별 값을 적용 — 일괄 동일 높이 금지
     예: item1: h-520px, item2: h-420px, item3: h-720px ... */
}
/* ⚠️ 타임라인 아이템에 gap을 사용하면 도트↔커넥터↔텍스트 사이에도 간격이 생긴다.
   아이콘과 연도 사이에만 margin-top으로 간격을 주고,
   도트+수직커넥터+텍스트는 하나의 서브 컨테이너(flex-column, gap 없음)로 묶어
   연속으로 붙어 있게 한다. gap을 전체 컬럼에 일괄 적용하지 않는다. */
/* 아이콘 컨테이너 — 배경 모양(Union/shield) 위에 아이콘 이미지 오버레이 */
.timeline-icon {
  position: relative;
  width: 100px;
  height: 108px;
}
.timeline-icon-bg {  /* Union/shield 배경 이미지 — get_screenshot으로 캡처 권장 */
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
}
.timeline-icon-img {  /* 실제 아이콘 이미지 */
  position: absolute;
  left: 19px;
  top: 18px;
  width: 60px;
...
/* 도트 — 큰 원형 (44px), 절대 작은 점(4~5px)으로 축소하지 않는다 */
/* ⚠️ 도트의 fill/stroke를 반드시 스크린샷에서 확인한다:
   - 흰색 내부 + 진한 테두리(border) → 링 스타일
   - 단색 채움(solid fill) → 솔리드 스타일
   MCP 데이터에 도트가 이미지 에셋(imgEllipse 등)인 경우, 해당 이미지를 사용하거나
   스크린샷을 보고 CSS로 동일하게 재현한다. 솔리드와 링을 혼동하지 않는다. */
.timeline-dot-area {
  position: relative;
  width: 44px;
  height: 44px;
...
.timeline-dot {
  width: 44px;  /* MCP 데이터의 정확한 크기 — 축소 금지 */
  height: 44px;
  border-radius: 50%;
  /* 스크린샷 기준으로 아래 중 택1:
     링 스타일: background: white; border: 4px solid #0f4c5c;
     솔리드:    background: #0f4c5c; */
}
/* 도트 간 수평 연결선 — 도트 중심 높이에 absolute 배치 */
.timeline-connector-h {
  position: absolute;
  height: 4px;  /* MCP 데이터에서 추출 */
  width: 120px;  /* 좌우 각각 120px — 도트 중심에서 인접 도트까지 */
}
/* 수직 커넥터 — 도트와 텍스트를 연결, 높이가 아이템마다 다름 */
.timeline-connector-v {
  width: 2px;
  flex: 1;  /* 남은 공간을 채워서 아이템 height에 따라 길이가 달라짐 */
}
/* 타임라인 수평선 (전체 관통) — 도트의 중심 높이에 absolute로 배치 */
.timeline-line {
  position: absolute;
  left: 0;
  right: 0;
...
/* 텍스트 박스 — MCP 데이터의 font-size, padding 정확히 적용 */
/* ⚠️ whitespace-nowrap 텍스트는 153px 컬럼을 넘어 오버플로한다.
   부모 컨테이너(.timeline)와 섹션 컨테이너에 overflow: visible 또는
   overflow-x: clip (수평 스크롤 방지)을 적용한다.
   양쪽 끝 아이템의 텍스트가 섹션 padding 밖으로 넘칠 수 있으므로,
   섹션 컨테이너에 overflow: hidden을 적용하여 깔끔하게 클리핑하거나,
   타임라인 컨테이너 자체에 좌우 여백을 추가하여 텍스트가 잘리지 않게 한다. */
.timeline-text {
  font-size: 24px;  /* MCP 데이터에서 추출한 정확한 값 — 14px 등으로 축소 금지 */
  line-height: 34px;
  padding: 40px;
  text-align: center;
  white-space: nowrap;  /* MCP 데이터에 nowrap이 있으면 그대로 적용 */
}
...
/* ⚠️ 스티키 탭/서브 네비게이션 — 정렬 방식을 MCP 데이터에서 정확히 추출 */
/* 헤더(GNB)와 서브 탭의 정렬이 다른 경우가 매우 흔하다:
   - 헤더(GNB): justify-between (로고 좌측, 메뉴 중앙, 버튼 우측)
   - 서브 탭: justify-center (탭 메뉴 + 버튼이 함께 중앙 정렬)
   MCP 출력의 justify-content 값을 반드시 확인하고 그대로 적용한다.
   헤더가 space-between이라고 서브 탭도 동일하게 가정하지 않는다. */
.sticky-tab {
  display: flex;
  justify-content: center;  /* MCP 데이터에서 정확히 추출 — center vs space-between */
  align-items: center;
  gap: 24px;  /* nav와 버튼 사이 gap — MCP 데이터에서 추출 */
  padding: 0 320px;  /* 서브 탭은 page-padding과 같을 수도, 다를 수도 있다 */
  backdrop-filter: blur(5px);
...
- **프로젝트에 이미 존재하는 이미지 파일이라도 Figma 디자인과 다르면 반드시 Figma 에셋으로 교체한다** — 기존 파일의 색상, 형태, 스타일이 Figma 스크린샷과 일치하는지 비교 필수

**로고/아이콘 에셋 처리 (중요):**

Figma에서 로고가 여러 SVG 파트(Union, Vector 등)로 분할되어 있는 경우가 매우 흔하다. 이 경우 개별 파트를 조립하면 위치나 비율이 어긋나기 쉽다.

1. **로고 노드 전체를 `get_screenshot`으로 캡처**하여 단일 이미지로 사용하는 것을 우선한다
   ```
   get_screenshot(fileKey, nodeId="로고-컨테이너-ID")
   ```
2. 스크린샷 이미지를 프로젝트에 저장하고 `<img>` 태그로 렌더링한다
3. 개별 SVG 파트를 absolute positioning으로 조립하는 방식은 **피한다** — 브라우저 렌더링에서 미세한 차이가 발생한다
4. **프로젝트에 기존 로고 파일이 있더라도** Figma 디자인의 로고와 시각적으로 다르면(색상, 형태, 비율 등) **반드시 Figma에서 새로 가져온 에셋으로 교체**한다
...
**비교 체크리스트:**
- [ ] **텍스트 콘텐츠가 Figma 원문과 정확히 일치하는가** — 한 글자도 임의로 변경/요약하지 않았는지 확인. 줄바꿈 위치도 동일해야 함
- [ ] 레이아웃 구조가 동일한가 (가로/세로 배치, 비율)
- [ ] **카드/리스트 내부 아이콘 이미지가 포함되었는가** — Figma 카드에 아이콘이 있으면 반드시 구현에도 포함
- [ ] **카드 내부 레이아웃 방향이 맞는가** — 아이콘+텍스트 가로 배치(flex-row) vs 세로 배치(flex-column)
- [ ] **정렬 방식이 정확한가** (justify-center vs justify-between vs flex-start) — **헤더와 서브 탭의 정렬이 다를 수 있음**
- [ ] **flex-wrap이 필요한 곳에 적용되었는가** — Figma의 `content-center flex flex-wrap`은 반응형 래핑을 의미
...
- [ ] 이미지/로고가 올바르게 표시되는가 — **Figma 에셋에서 가져온 원본 사용, 프로젝트 기존 파일과 Figma 디자인이 다르면 교체**
- [ ] 보더, 그림자, radius가 맞는가
...
### 타임라인/연혁 구조가 디자인과 다름
타임라인은 가장 자주 잘못 구현되는 컴포넌트이다. 아래 체크리스트를 반드시 확인한다.

**구조 오류 (가장 흔함):**
→ ❌ "가로 행 3줄 분할" (아이콘 행 → 도트 행 → 텍스트 행)로 구현하면 안 된다.
→ ✅ "컬럼별 아이템" (각 아이템이 독립적 flex-column)으로 구현한다.
→ MCP 데이터에서 각 아이템이 `flex flex-col`인지 확인 — 이것이 올바른 구조의 증거이다.

**도트/커넥터 오류:**
→ 도트 크기를 MCP 데이터 그대로 적용한다 (예: 44px). 4~5px 작은 점으로 축소하지 않는다.
→ 도트의 fill/stroke를 스크린샷에서 확인한다 — 흰색 내부+테두리(링) vs 단색 채움(솔리드)을 구분한다.
→ 도트 간 수평 연결선(120px)이 있는지 MCP 데이터를 확인한다 — 없으면 추가하지 않지만, 있으면 반드시 구현한다.
→ 수직 커넥터(세로선)도 MCP 데이터에 있으면 구현한다 — 아이템별 높이 차이는 이 커넥터 길이로 표현된다.
→ **도트↔수직커넥터↔텍스트는 간격 없이 연속으로 연결**되어야 한다. 컬럼 전체에 `gap`을 적용하면 도트와 커넥터 사이에도 간격이 생기므로, 도트+커넥터+텍스트를 하나의 서브 컨테이너로 묶고 아이콘/연도와의 간격만 `margin`으로 처리한다.

**아이템별 높이 오류:**
→ MCP 데이터에서 동일 컴포넌트의 각 인스턴스 height가 다르면 (예: 520, 420, 720, 460...) **각각 개별 height를 적용**한다.
→ 모든 아이템을 동일 높이로 만들지 않는다.
→ height 차이는 커넥터 길이와 텍스트 줄 수에서 발생한다 — 지그재그 패턴이 아니다.

**아이콘 배경 오류:**
→ 타임라인 아이콘에 배경 모양(Union/shield 등)이 있는지 MCP 데이터를 확인한다.
→ 배경 모양이 있으면 position relative 컨테이너 안에 배경 이미지와 아이콘 이미지를 오버레이한다.

**텍스트/폰트 크기 오류:**
→ MCP 데이터의 font-size를 그대로 사용한다 (예: 24px). 14px 등으로 임의 축소하지 않는다.
→ 텍스트 내용은 MCP 출력의 원문 그대로 사용한다 — 연혁 텍스트를 요약/변경하지 않는다.
...
### 텍스트 내용이 디자인과 다름
구현 결과의 텍스트가 Figma 디자인의 텍스트와 다른 경우가 있다.
→ MCP 출력 코드의 텍스트를 **한 글자도 바꾸지 않고 그대로** 사용한다.
→ "더 자연스럽게", "더 잘 읽히게" 등의 이유로 텍스트를 의역하거나 요약하지 않는다.
→ 줄바꿈(`<br>`)의 위치도 Figma 원본 그대로 유지한다.
→ 텍스트 내용은 기획자/디자이너의 의도이므로, 변환 과정에서 변경하면 안 된다.

### 카드에 아이콘 이미지가 누락됨
피처 카드, 서비스 설명 카드 등에 아이콘 이미지가 빠지는 경우가 흔하다.
→ MCP 출력에서 카드 컴포넌트(`component/Body/T2` 등) 내부의 **이미지 노드**를 반드시 확인한다.
→ `image/service.icon.01` 같은 에셋 이름이 보이면 해당 URL을 추출하여 적용한다.
→ 카드의 **내부 레이아웃 방향**(아이콘+텍스트 가로 배치 vs 세로 배치)을 MCP 출력의 `flex-row` / `flex-col`에서 확인한다.
→ 아이콘 크기(width/height), 모양(border-radius: 원형 여부), 이미지 피팅(object-cover, overflow:hidden)을 정확히 적용한다.

### 로고 이미지가 디자인과 다름
Figma의 로고가 여러 SVG 파트(Union, Vector 등)로 구성된 경우가 있다.
→ 로고 컨테이너 노드에 대해 `get_screenshot`을 호출하여 **단일 이미지로 캡처**한다.
→ 개별 SVG 파트를 absolute positioning으로 조립하지 않는다 — 렌더링 차이가 발생한다.
→ 로고를 텍스트나 대체 이미지로 임의 구현하지 않는다.
→ **프로젝트에 기존 로고 파일이 있어도 Figma 디자인과 다르면 교체한다** — 색상(그라데이션 vs 단색), 형태, 비율을 Figma 스크린샷과 비교한다.
→ 캡처한 이미지가 Figma 스크린샷과 동일한지 비교 확인한다.
